...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'awaiting_phone':
        ...
        if not user.get('welcome_given'):
            await db.adjust_balance(uid, 10.0, 'welcome_bonus', '10 TK welcome bonus')
            await db.update_user(uid, welcome_given=1)
            msg = t('reg_done_bonus', lang)
        else:
            msg = t('reg_done', lang)
        await db.set_state(uid, None)
        await update.message.reply_text(msg, reply_markup=MKB)
        ref = user.get('referrer_id')
        if ref and ref != uid:
            await db.adjust_balance(ref, config.REFERRAL_BONUS, 'referral_bonus')
            rlang = await db.get_user_lang(ref)
            try:
                await context.bot.send_message(ref, t('referral_bonus', rlang, bonus=config.REFERRAL_BONUS))
            except Exception:
                pass
        return
