...
async def get_index():
    # website.html খোঁজো
    for path in [
        os.path.join(os.path.dirname(__file__), "website.html"),
        os.path.join(os.path.dirname(__file__), "templates", "index.html"),
    ]:
        if os.path.exists(path):
            return FileResponse(path)
    return HTMLResponse("<h1>eFootball Bot is running! ✅</h1>")
...
async def get_leaderboard():
    ...
    async with aiosqlite.connect(db.DB) as conn:
        ...
        async with conn.execute(
            "SELECT ingame_name, elo, wins, losses FROM users WHERE is_registered=1 ORDER BY elo DESC LIMIT 10"
        ) as cur:
...
async def get_tournaments():
    ...
    async with aiosqlite.connect(db.DB) as conn:
        conn.row_factory = aiosqlite.Row
        async with conn.execute(
            "SELECT * FROM tournaments WHERE status IN ('OPEN','RUNNING') ORDER BY created_at DESC LIMIT 10"
        ) as cur:
            rows = await cur.fetchall()
    result = []
    for r in rows:
        t = dict(r)
        async with aiosqlite.connect(db.DB) as conn2:
            async with conn2.execute(
                "SELECT COUNT(*) as c FROM tournament_players WHERE tourney_id=?", (t['id'],)
            ) as cur2:
                cnt = await cur2.fetchone()
        t['joined'] = cnt[0] if cnt else 0
        result.append(t)
    return result
